...
def auth(request):
    ...
    if login_form.is_valid():
        ...
        if user is not None:
            ...
            hora_sesion = datetime.utcnow().replace(tzinfo=ZoneInfo("UTC"))
            zona = ZoneInfo("Chile/Continental")
            local_time = hora_sesion.astimezone(zona)
            session['login_date'] = local_time.strftime("%d-%m-%Y %H:%M:%S")
